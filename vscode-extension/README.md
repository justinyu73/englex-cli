...
The input-box command is `englex.lookupInput` and uses `Ctrl+Alt+L` (`Cmd+Alt+L` on macOS). The selection command keeps `Ctrl+Alt+E` and its editor-context menu. A single **$(book) Englex** status-bar button is available after VS Code startup; click it to choose either local lookup or the maintainer-only wishlist batch. The button shows the current net-new wishlist count when available.
...
In the Extension Development Host, invoke **Englex: Look Up Engineering Term** from the Command Palette, press `Ctrl+Alt+L`, or click the **Englex** status-bar button and choose **Englex 查詞**, then type or paste a term into the input box and press Enter. The same menu exposes **Englex 補批（維護者）**; it remains protected by the existing maintainer configuration and explicit confirmation. You can also create a text file, explicitly select one line, and invoke **Englex: Explain Selected Engineering Terms** from the editor context menu, Command Palette, or `Ctrl+Alt+E`. Both results appear in the **Englex Selection** output channel. The CLI remains the authority for the 200-character single-line input validation and all result semantics.
